...
│   ├── report_writer.py        # Xuất báo cáo kỹ thuật (.md/.txt): R1–R8, hệ số sử dụng
...
| Báo cáo kỹ thuật (.md/.txt) | Hệ số sử dụng, tổ hợp chi phối, bảng R1–R8 |
...
`core/rigid_cap.py` là nguồn duy nhất của công thức bệ cứng. Nó **không** tham gia quyết định thiết kế, chỉ phục vụ: tô **heatmap**, chế độ **mock** (xem trước khi chưa cấu hình MCOC) và **dự báo dẫn hướng** trong Refine. Lực dọc trục trên cọc thứ *i*:

```
Pᵢ = N/n + Mx·(yᵢ − cy)/Ix + My·(xᵢ − cx)/Iy
```

với `cx, cy` là tâm nhóm cọc, `Ix = Σ(yᵢ−cy)²`, `Iy = Σ(xᵢ−cx)²`. Vì lưới do `generator.py` sinh ra luôn đối xứng quanh gốc (`cx = cy = 0`) nên số hạng chuyển momen triệt tiêu. Tính tức thì (<0.1 ms) nhưng là **ước lượng** — mọi số giao nộp vẫn lấy từ MCOC.
...
Để bám sát yêu cầu kỹ thuật (TCVN 10304:2014 / TCVN 11823), mô hình bổ sung:
...
`nsga2_optimizer` và `refine_optimizer` dùng chung `core.rigid_cap` nên các ràng buộc áp dụng nhất quán. Báo cáo kỹ thuật (`io_handlers/report_writer.py`) in đầy đủ R1–R8 kèm **hệ số sử dụng** và **tổ hợp chi phối**; mẫu xem `docs/ban_output_chuan_ky_thuat.md`.
