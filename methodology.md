...
Bộ tối ưu chính là **NSGA-II (giải thuật di truyền đa mục tiêu)**, mỗi phương án được **chấm bằng MCOC chính xác** (gọi `MCOC_Batch.exe` như một hộp đen). Hai mục tiêu cực tiểu đồng thời: **số cọc** và **mục tiêu phụ** (bệ gọn — mặc định, hoặc Pmax nhỏ). Kết quả trả về **mặt Pareto** để kiến nghị phương án.

> **Lưu ý:** công thức **bệ cứng** chỉ còn dùng phụ trợ (xem trước nhanh, tô heatmap, dự báo dẫn hướng cho Refine), **không** tham gia quyết định thiết kế. Mọi nội lực giao nộp đều do MCOC tính.
...
> **Luồng quyết định (chung cho Tab Tương tác và Hàng loạt):** nút "Chạy tối ưu hóa" → `run_nsga2()` với evaluator MCOC thực (`mcoc_writer` sinh input → `MCOC_Batch.exe` → đọc `_result.txt`). Mỗi phương án được chấm bằng MCOC. **Bắt buộc** cấu hình MCOC + file input gốc; thiếu thì từ chối chạy.
>
> Mô hình bệ cứng **không** nằm trên luồng này — chỉ dùng tô heatmap khi hiển thị và làm bộ dự báo dẫn hướng.
...
Mỗi phương án (một cá thể NSGA-II) được mã hóa bởi `(kiểu lưới, nx, ny, sx, sy)` rồi sinh tọa độ căn giữa gốc theo **2 kiểu lưới**:
...
NSGA-II tiến hóa quần thể phương án qua nhiều thế hệ (lựa chọn → lai tạo → đột biến → chấm điểm bằng MCOC → xếp hạng Pareto), trả về **mặt Pareto** cân bằng giữa **số cọc** và **độ gọn của bệ**. Phương án kiến nghị là điểm tốt nhất trên mặt Pareto (ưu tiên ít cọc nhất, rồi bệ gọn nhất). Chi tiết thuật toán ở **§4.6**.
...
- Công thức **bệ cứng** chỉ là *heuristic dẫn hướng* (xếp hạng ứng viên, giải nhị phân bước cọc) + tô **heatmap**; nó ảnh hưởng **thứ tự** duyệt chứ không ảnh hưởng kết quả ⇒ tắt được mà không đổi tính đúng đắn.
...
`core/rigid_cap.py` là nguồn duy nhất của công thức bệ cứng. Nó **không** tham gia quyết định thiết kế, chỉ phục vụ: tô **heatmap**, chế độ **mock** (xem trước khi chưa cấu hình MCOC) và **dự báo dẫn hướng** trong Refine. Lực dọc trục trên cọc thứ *i* (tải `N` đặt tại gốc tọa độ, **dời mômen về trục trọng tâm nhóm cọc**):

```
Pᵢ = N/n + (Mx − N·cy)·(yᵢ − cy)/Ix + (My − N·cx)·(xᵢ − cx)/Iy
```

với `cx, cy` là tâm nhóm cọc, `Ix = Σ(yᵢ−cy)²`, `Iy = Σ(xᵢ−cx)²` (xem [`rigid_cap.pile_forces`](core/rigid_cap.py:21)).
...
Đây là **bộ tối ưu mặc định** của ứng dụng (Non-dominated Sorting Genetic Algorithm II, Deb et al. 2002). Khác với Refine (vòng lặp tất định), NSGA-II tiến hóa một **quần thể** phương án và trả về **toàn bộ mặt Pareto** cân bằng giữa số cọc và mục tiêu phụ. Nút "Chạy tối ưu hóa" gọi engine này với evaluator **MCOC exact**.
...
Mỗi genome chỉ gọi evaluator **một lần** nhờ **cache theo `spec_key`** (vì GA thường xét lại cùng cấu hình); tham số `max_evals` đặt trần số lần chạy MCOC để kiểm soát thời gian. Kết quả trả về tương thích với `run_optimization` (`recommended`, `all_valid_configs`, `best_A/B`) và bổ sung `pareto_front`, `n_evals`, `eval_mode`.
...
**Phân phối lực ngang Hx, Hy, Mz** (`rigid_cap.horizontal_forces`) — trước đây công thức bệ cứng chỉ phân phối lực dọc trục và bỏ qua Hx, Hy, Mz dù người dùng có nhập. Nay với giả thiết cọc đứng độ cứng ngang bằng nhau:
...
Đường chính: ghép `mcoc_writer` + `mcoc_runner` thành một hàm `evaluator(coords)` chạy MCOC thực và trả về `{pmax, pmin, mxmax, mymax}` (Tấn). Đây là evaluator truyền vào `run_nsga2` và `run_pareto_refinement`.

#### Chế độ mock (xem trước)

Khi chưa cấu hình MCOC, `evaluate_layout()` dùng công thức bệ cứng (`core/rigid_cap.py`) nhân hệ số hiệu chỉnh `K = Pmax_MCOC(gốc) / Pmax_bệ_cứng(gốc)` để ước lượng nhanh — chỉ để xem trước, không dùng cho hồ sơ.

> **Hạn chế của mock (chỉ ảnh hưởng xem trước, KHÔNG ảnh hưởng hồ sơ vì hồ sơ dùng MCOC):**
> - **Mômen đầu cọc** ở mock ước lượng bằng heuristic `Mx ≈ Mx_gốc · (n_gốc / n)` (tỉ lệ nghịch số cọc) — **không** suy ra từ cơ học bệ cứng (bệ cứng chỉ cho lực dọc trục, không cho mômen thân cọc). Vì vậy nếu bật kiểm tra `[M]` (R6) ở **chế độ mock**, độ tin cậy R6 thấp hơn R1/R2 → chỉ nên tin R6 khi chấm bằng **MCOC**. Chương trình **tự cảnh báo** điều này khi chạy mock với `[M] > 0` (`run_nsga2` ghi log "CANH BAO…"; `run_optimization` trả về trường `warning` và `run_demo.py` in ra).
> - Hệ số `K` trích từ phương án gốc (thường đối xứng) rồi nhân cho mọi phương án: chỉ co giãn theo tỉ lệ, **không** sửa được khác biệt cấu trúc ở layout lệch tâm. Đây là lý do mock chỉ để xem trước.
...
`run_optimization(params, loads)` quét toàn bộ lưới A/B bằng công thức bệ cứng và trả về `recommended`, `all_valid_configs`, `best_A/B`, `original_config`. **Chỉ dùng cho `run_demo.py`** (xem nhanh, không cần MCOC); không nằm trên luồng quyết định thiết kế.

> **Ràng buộc đường chéo Kiểu B (ĐÃ SỬA):** ràng buộc gần nhất của Kiểu B là **đường chéo** `√((sx/2)² + sy²)`, ở bước lớn nhất thường vượt 6d (vd `sx=sy=6d → diag≈6.7d`). Trước đây demo đặt `sy=sy_max` cố định nên nhiều Kiểu B bị **loại oan**. Nay demo **giảm `sy`** xuống `√(36d² − (sx/2)²)` để kéo đường chéo về đúng 6d (vẫn giữ `sy` lớn nhất → mômen quán tính lớn), nên Kiểu B được khảo sát đầy đủ (đã kiểm: tìm được nhiều phương án B với `diag ∈ [3d,6d]`). Engine quyết định **NSGA-II** vốn không vướng lỗi này vì tối ưu `sx, sy` liên tục và đo khoảng cách bằng `min_spacing` thực.

---

### 5.4b `core/refine_optimizer.py` — Tinh chỉnh Pareto + MCOC thực

Module này hiện thực **đường tính toán thứ hai** (tính năng *Refine* trên UI), khác với `optimizer.py` (quét lưới nhanh bằng mock). Nó **gọi MCOC thực** để kiểm chứng thay vì chỉ ước lượng.

#### `run_pareto_refinement(params, loads, evaluator, log, budget=25) → dict`

Hàm chính được `ui/main_window.run_refine_real()` gọi. Nguyên tắc *predict – verify – recalibrate*:
...
3. Lấy ứng viên tốt nhất trên mặt Pareto **(số cọc, độ gọn bệ = footprint)**, gọi MCOC thực kiểm chứng. Nếu KHÔNG ĐẠT → lưu hệ số hiệu chỉnh **riêng** cho cấu hình đó (`cfg_K`) để vòng sau tự đẩy lùi dự báo.
...
Tinh chỉnh cục bộ quanh lưới gốc bằng các bước rời rạc: co `sx`/`sy` một nấc, bỏ 1 cột, bỏ 1 hàng, hoặc chuyển lưới A → hoa mai B. Được dùng trong `tests/test_refine.py`.
...
Hai module này được `MCOCBlackbox.make_real_evaluator()` (trong `blackbox.py`) ghép lại thành `evaluator(coords) → dict` — chính là `evaluator` truyền vào `run_pareto_refinement` **và** `run_nsga2`.
...
Vòng lặp NSGA-II hoàn chỉnh: khởi tạo quần thể ngẫu nhiên → (tournament → SBX → mutation → đánh giá → gộp cha+con → non-dominated sort + crowding → chọn lọc μ+λ) lặp `n_gen` thế hệ. Trả về `recommended`, `pareto_front`, `all_valid_configs`, `n_evals`, `eval_mode`. Nếu `evaluator=None` → tự dùng mock (`make_mock_evaluator`); truyền evaluator MCOC thực để đánh giá exact.
...
| **Tải trọng** | N=2577 T, Mx=1500, My=1500 | 1 tổ hợp mặc định  |
...
> Dự án **đã có sẵn** script demo `run_demo.py` với nội dung tương đương (kèm in bảng nội lực từng cọc). Có thể chạy ngay `python run_demo.py`. Đoạn dưới là phiên bản tối giản để tham khảo/tùy biến — nếu muốn lưu riêng, đặt tên bất kỳ, ví dụ `my_test.py`:
...
> Lưu ý: phương án gốc trong ví dụ này **KHÔNG ĐẠT** vì cả hai lý do: `Pmax = 519.63 T > Po = 500 T` và khoảng cách tim–tim 3.00 m < `3d = 3.60 m` (với d = 1.2 m). Đây cũng là minh họa cho việc `check_layout` **gom đủ mọi lỗi** thay vì dừng ở lỗi đầu tiên.
...
*Cập nhật: 2026-06-15 (chuyển các sơ đồ luồng sang Mermaid).*
